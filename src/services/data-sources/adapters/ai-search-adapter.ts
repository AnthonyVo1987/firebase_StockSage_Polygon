/**
 * @fileOverview Adapter for fetching stock data using an AI with search capabilities.
 */
...
const INPUT_PRICE_PER_MILLION_TOKENS = 0.15; // Keep consistent
...
export class AISearchAdapter implements IDataSourceAdapter {
  constructor() {
    console.log('[ADAPTER:AISearch] AISearchAdapter instantiated.');
  }

  async getFullStockData(ticker: string): Promise<AdapterOutput> {
    console.log(`[ADAPTER:AISearch] getFullStockData called for ticker: ${ticker}. Fetching live data via AI.`);
    
    const fetchInput: FetchStockDataInput = { ticker, forceMock: false }; // Always false for live AI search
    try {
      const flowResult = await fetchStockData(fetchInput);
      const usageReport = calculateAiUsageReport('fetchStockDataFlow (AISearchAdapter)', flowResult.usage);

      if (flowResult.error || !flowResult.data?.stockJson) {
        const errorMsg = flowResult.error || 'AI could not retrieve stock data via search.';
        console.error(`[ADAPTER:AISearch] Error: ${errorMsg}`);
        return {
      ...
      try {
        ...
        const errorMsg = `AISearchAdapter: AI returned invalid JSON or JSON missing critical fields: ${e.message}`;
        console.error(`[ADAPTER:AISearch] ${errorMsg}`, flowResult.data.stockJson);
        return {
      ...
      const errorMsg = `AISearchAdapter: Unexpected error calling fetchStockData flow: ${e.message}`;
      console.error(`[ADAPTER:AISearch] ${errorMsg}`, e);
      return {
        stockDataJson: { ...EMPTY_STOCK_DATA_JSON },
        error: errorMsg,
      };
